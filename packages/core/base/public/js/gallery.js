...
/*!*****************************************************************!*\
  !*** ./packages/core/base/resources/assets/js/admin/gallery.js ***!
  \*****************************************************************/
$(document).ready(function () {
  $('.btn_select_gallery').__Media({
    filter: 'image',
    view_in: 'all_media',
    onSelectFiles: function onSelectFiles(files) {
      var galleryItem = $('.list-photos-gallery .photo-gallery-item:last-child');
      var last_index = galleryItem.length > 0 ? galleryItem.data('id') + 1 : 1;
  ...
  $listPhotos.on('click', '.photo-gallery-item', function () {
    ...
    $('#delete-gallery-item').attr('data-id', id);
    $('#update-gallery-item').attr('data-id', id);
  ...
  $editGalleryItem.on('click', '#delete-gallery-item', function (event) {
    ...
    $listPhotos.find('.photo-gallery-item[data-id=' + $(this).attr('data-id') + ']').remove();
    updateItems();
  ...
  $editGalleryItem.on('click', '#update-gallery-item', function (event) {
    event.preventDefault();
    $editGalleryItem.modal('hide');
    $listPhotos.find('.photo-gallery-item[data-id=' + $(this).attr('data-id') + ']').data('description', $('#gallery-item-description').val());
    updateItems();
  });
});
/******/ })()
