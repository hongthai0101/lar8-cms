...
use Messi\Media\Http\Resources\FolderResource;
use Messi\Media\Models\MediaFile;
use Messi\Media\Models\MediaFolder;
...
use Messi\Media\Supports\Zipper;
use Eloquent;
use Exception;
use File;
...
class MediaController extends Controller
{
    ...
    {
        $response = Media::responseError(trans('core/media::media.invalid_action'));
        ...
        switch ($type) {
            case 'trash':
                ...
                if ($error) {
                    $response = Media::responseError(trans('core/media::media.trash_error'));
                    break;
                }

                $response = Media::responseSuccess([], trans('core/media::media.trash_success'));
                break;
            ...
            case 'restore':
                ...
                if ($error) {
                    $response = Media::responseError(trans('core/media::media.restore_error'));
                    break;
                }

                $response = Media::responseSuccess([], trans('core/media::media.restore_success'));
                break;
            ...
            case 'make_copy':
                ...
                $response = Media::responseSuccess([], trans('core/media::media.copy_success'));
                break;
            ...
            case 'delete':
                ...
                $response = Media::responseSuccess([], trans('core/media::media.delete_success'));
                break;
            ...
            case 'favorite':
                ...
                $response = Media::responseSuccess([], trans('core/media::media.favorite_success'));
                break;
            ...
            case 'remove_favorite':
                ...
                $response = Media::responseSuccess([], trans('core/media::media.remove_favorite_success'));
                break;
            ...
            case 'rename':
                ...
                if (!empty($error)) {
                    $response = Media::responseError(trans('core/media::media.rename_error'));
                    break;
                }

                $response = Media::responseSuccess([], trans('core/media::media.rename_success'));
                break;
            ...
            case 'empty_trash':
                ...
                $response = Media::responseSuccess([], trans('core/media::media.empty_trash_success'));
                break;
        }
    ...
    protected function copyFile($file, $newFolderId = null)
    {
        ...
        if ($newFolderId == null) {
            ...
            $filePath = Media::getRealPath($file->url);
            if (Storage::exists($filePath)) {
                ...
                Media::generateThumbnails($file);
            }
        } else {
            $file->url = str_replace(
                Media::getRealPath(File::dirname($file->url)),
                Media::getRealPath($this->folderRepository->getFullPath($newFolderId)),
                $file->url
            );
    ...
    public function download(Request $request)
    {
        ...
        if (count($items) == 1 && $items['0']['is_folder'] == 'false') {
            ...
            if (!empty($file) && $file->type != 'video') {
                $filePath = Media::getRealPath($file->url);
                if (!Media::isUsingCloud()) {
                    if (!File::exists($filePath)) {
                        return Media::responseError(trans('core/media::media.file_not_exists'));
                    }
            ...
            $fileName = Media::getRealPath('download-' . now()->format('Y-m-d-h-i-s') . '.zip');
            $zip = new Zipper;
            $zip->make($fileName);
            foreach ($items as $item) {
                ...
                if ($item['is_folder'] == 'false') {
                    ...
                    if (!empty($folder)) {
                        if (!Media::isUsingCloud()) {
                            $zip->add(Media::getRealPath($this->folderRepository->getFullPath($folder->id)));
                        } else {
                            $allFiles = Storage::allFiles($this->folderRepository->getFullPath($folder->id));
                            foreach ($allFiles as $file) {
                                $zip->addString(File::basename($file),
                                    file_get_contents(str_replace('https://', 'http://', Media::getRealPath($file))));
                            }
                        }
                    }
                }
            ...
            return Media::responseError(trans('core/media::media.download_file_error'));
        }

        return Media::responseError(trans('core/media::media.can_not_download_file'));
    }
}
